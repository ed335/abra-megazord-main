...
import { ChevronLeft, HeartHandshake, User, MapPin, FileText, Check } from 'lucide-react';
...
export default function CadastroParceiroClient() {
  ...
  const validateStep = (step: number): boolean => {
    ...
    if (step === 1) {
      ...
      if (!formData.tipoParceria.trim()) errors.tipoParceria = 'Tipo de parceria é obrigatório';
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (data.success) {
        setStatus('success');
        router.push('/cadastro/sucesso?tipo=parceiro');
      } else {
        setStatus('error');
  ...
  const steps = [
    { id: 1, title: 'Dados da Organização', icon: HeartHandshake },
        ...
        <div className="flex justify-between items-start mb-8">
          <div>
            ...
            <p className="text-sm text-cinza-medio mt-1">
              Torne-se uma organização parceira da ABRACANM
            </p>
          </div>
        </div>
            ...
            <AnimatePresence mode="wait" custom={direction}>
              {currentStep === 1 && (
                <motion.div
                  ...
                  className="space-y-4"
                >
                  <h2 className="text-xl font-semibold text-cinza-escuro mb-4">Dados da Organização</h2>
                  
                  <label className="flex flex-col gap-1.5">
                    <span className={labelClass}>Tipo de Parceria *</span>
                    <select
                      value={formData.tipoParceria}
                      onChange={(e) => updateField('tipoParceria', e.target.value)}
                    ...
                    {fieldErrors.tipoParceria && <span className="text-xs text-red-500">{fieldErrors.tipoParceria}</span>}
                  </label>
              ...
              {currentStep === 2 && (
                <motion.div
                  ...
                  className="space-y-4"
                >
                  <h2 className="text-xl font-semibold text-cinza-escuro mb-4">Responsável pelo Contato</h2>
              ...
              {currentStep === 4 && (
                ...
                >
                  ...
                  <div className="bg-verde-claro/10 rounded-lg p-4 space-y-2">
                    <div className="inline-block bg-sucesso/20 text-sucesso text-xs font-medium px-2 py-1 rounded mb-2">
                      {formData.tipoParceria}
                    </div>
